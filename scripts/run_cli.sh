...
VIDEO_FILE="/data/videos/huberman_tools_to_accelerate_your_fitness_goals.mp4"
OUTPUT_DIR="/data/videos/huberman_tools_to_accelerate_your_fitness_goals"
...
if ! docker exec "$CONTAINER_NAME" /bin/bash -c "chmod -R 777 ${OUTPUT_DIR}"; then
    echo "Failed to change file mod!"
    exit 1
fi
